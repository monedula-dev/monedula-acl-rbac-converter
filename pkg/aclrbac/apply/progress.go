...
//
// apply.Run calls Printf from parallel goroutines, so the underlying writer
// access is serialized through mu. Even when the writer is os.Stderr (which
// the OS serializes on write(2)), interleaved fmt.Fprintf arguments would
// produce garbled lines; mu prevents that as well.
