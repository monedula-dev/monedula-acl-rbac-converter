...
func writeRollback(opts Options, rows []types.ACLRow, dest string) error {
	...
	b.WriteString(common.BuildScriptHeader(common.HeaderInputs{
		ToolVersion:      version.Version + " (rollback)",
...
func writeAdd(b *strings.Builder, r types.ACLRow, opts Options) {
	args := []string{
	...
	// See note in script.go: Quote everything except the binary. The previous
	// `HasPrefix(a, "--")` exemption was a shell-injection vector for any
	// operator-supplied value starting with "--".
