#!/usr/bin/env bash
# setup-script: convert a kafka-acls provisioning shell script into RBAC.
#
...
mkdir -p out
echo "==> extract --from script (--vars vars.yaml substitutes \$ORDERS_TOPIC)"
conv extract --from script --input setup-acls.sh --vars vars.yaml --out out/acls.json
